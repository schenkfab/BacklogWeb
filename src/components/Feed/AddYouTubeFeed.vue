  ...
  <form>
    <div class="md:flex md:items-center mb-6">
      ...
      <div class="md:w-2/4 pr-4">
        <input
          placeholder="UCXVX_0ukCPMLPWk64KYnHFA"
      ...
      <div class="md:w-1/4 md:items-center">
        ...
        <div class="md:w-2/3">
          <button
            ...
            type="button"
            @click="getFeed"
          >Get YouTube Channel</button>
        </div>
      </div>
    </div>
    <div class="md:flex md:items-center mb-6" v-if="this.title">
      <div class="md:w-1/4">
        <label
          class="block text-gray-500 font-bold md:text-right mb-1 md:mb-0 pr-4"
          for="inline-full-name"
...
export default {
  ...
  methods: {
    ...
    async addFeed () {
      ...
      await this.addFeedAsync({ name: this.title, url: `https://www.youtube.com/feeds/videos.xml?channel_id=${this.url}` })
      this.created = true
      this.setLoading(false)
    },
    async getFeed () {
      const yturl = `https://www.youtube.com/feeds/videos.xml?channel_id=${this.url}`
      this.title = null
      this.created = false
      this.setLoading(true)
      try {
